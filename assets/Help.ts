...
module.exports = {
    ...
    execute: (msg: Message, args: Array<string>) => {
        ...
        // Show the full list
        if (!args.length) {
            const commandList: Array<string> = commands.map(command => `${command.name} | ${command.description}`)
        ...
        if (!command) return msg.reply(`${name} is an invalid command name, see ${config.prefix}help for a list of all possible commands`)
