...
module.exports = {
    ...
    execute: (msg, args) => {
        ...
        const isValidItem = Object.values(itemList).find((item) => item.name.toUpperCase() === itemToEquip.toUpperCase());
        if (isValidItem) {
            con.query('SELECT * FROM inventory WHERE id = ? AND name = ?;', [id, itemToEquip], (err, res) => {
                if (err) {
                    ...
                    con.query('UPDATE users SET equippedItem = ? WHERE id = ?;', [itemToEquip, id], (err) => {
                        if (err) {
                            fs.writeFileSync('./logs/ERR.log', `\n\n${err}`, { flags: "a" });
