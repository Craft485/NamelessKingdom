...
module.exports = {
    ...
    execute: (msg, args) => {
        ...
        con.query(`SELECT * FROM users WHERE id = ${id}; SELECT * FROM inventory WHERE id = ${id};`, (err, results) => {
            if (err) {
            ...
            if (inventoryData?.length === 0)
                return msg.reply('You feel light as you realize you aren\'t carrying anything');
            const items = [];
            inventoryData.forEach(item => {
                items.push(`${item.name} x${item.quantity}`);
            });
            const res = new discord_js_1.MessageEmbed({
