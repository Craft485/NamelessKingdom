...
module.exports = {
    ...
    execute: (msg, args) => {
        ...
        con.query('SELECT * FROM users WHERE id = ?;', [id], (err, data) => {
            ...
            if (!data || !data.length) {
                con.query('INSERT INTO users (id, name, attack, equippedItem) VALUES(?, ?, ?, \'stick\'); INSERT INTO inventory (id, name) VALUES (?, "stick")', [id, msg.author.username, JSON.stringify(itemList.stick.attack), id], (err) => {
                    if (err) {
                        fs.writeFileSync('./logs/ERR.log', `\n\n${err}`, { flags: "a" });
